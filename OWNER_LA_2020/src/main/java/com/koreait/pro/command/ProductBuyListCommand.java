...
public class ProductBuyListCommand implements ProCommand {
	...
	public void execute(SqlSession sqlSession, Model model) {
		...
		ArrayList<ProductDTO> pDTO = pDAO.selectProductBuyList(map);
		
		// 전체 게시글 개수 구하기
		int totalRecord = pDAO.getBuyAllTotalRecord(m_No);
		
		// 페이지 생성 ( <<  1  2  3  >> )
		String pageView = PageMaker.getPageView2("productBuyList",m_No ,Integer.parseInt(page), recordPerPage, totalRecord);
