...
public class ProductManagementCommand implements ProCommand {
	...
	public void execute(SqlSession sqlSession, Model model) {
		...
		ArrayList<ProductDTO> pDTO = pDAO.productManagement(map);
		
		// 전체 게시글 개수 구하기
		int totalRecord = pDAO.getManagementTotalRecord();

		// 페이지 생성 ( <<  1  2  3  >> )
		String pageView = PageMaker.getPageView("productManagementPage", Integer.parseInt(page), recordPerPage, totalRecord);
