...
public class Qna_qViewPage implements ProCommand {
	...
	public void execute(SqlSession sqlSession, Model model) {
		...
		ArrayList<Qna_QDTO> qDTO = qDAO.selectQNAList(map);

		// 전체 게시글 개수 구하기
		int totalRecord = qDAO.getUserTotalRecord(m_No);

		// 페이지 생성 ( <<  1  2  3  >> )
		String pageView = PageMaker.getPageView2("qnaViewPage", m_No, Integer.parseInt(page), recordPerPage, totalRecord);
