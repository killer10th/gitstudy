...
public class ProductListCommand implements ProCommand {
	...
	public void execute(SqlSession sqlSession, Model model) {
		...
		ArrayList<ProductDTO> pDTO = pDAO.selectProductList(map);

		// 전체 게시글 개수 구하기
		int totalRecord = pDAO.getAllTotalRecord();

		// 페이지 생성 ( <<  1  2  3  >> )
		String pageView = PageMaker.getPageView("productListPage", Integer.parseInt(page), recordPerPage, totalRecord);
