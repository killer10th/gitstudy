...
public class NoticeListCommand implements ProCommand {
	...
	public void execute(SqlSession sqlSession, Model model) {
		...
		ArrayList<NoticeDTO> nDTO = nDAO.selectNoticeList(map);

		// 전체 게시글 개수 구하기
		int totalRecord = nDAO.getAllTotalRecord();

		// 페이지 생성 ( <<  1  2  3  >> )
		String pageView = PageMaker.getPageView("NoticePage", Integer.parseInt(page), recordPerPage, totalRecord);
